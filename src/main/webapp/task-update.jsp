					...
					<div class="col-md-8 col-xs-12">
						<div class="white-box">
							<form action="update-task" method="post"
								class="form-horizontal form-material" accept-charset="UTF-8">
										...
										<select name="idProject"
											...
											<c:forEach items="${listProject}" var="project">
												<c:if test="${task.project.id == project.id}">
													<option value="${task.project.id}">${project.name}</option>
												</c:if>
											</c:forEach>
										</select>
									</div>
								...
								<div class="form-group">
									<div class="col-sm-12">
										<a class="btn btn-success btn-update" id-task="${task.id}">Cập
											nhật công việc</a> <a href="tasks" class="btn btn-primary">Quay
											lại</a>
									</div>
								</div>
							</form>
